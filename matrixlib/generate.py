...
def add_blocks(
    ...
    value_low: float = value_range[0]
    value_high: float = value_range[1]
    block_size_low: int = block_size_range[0]
    block_size_high: int = block_size_range[1]

    # create block size generator for truncated bell curve
    scale = int(block_size_average * block_size_std_dev)
    lower_bound_offset = (block_size_low - block_size_average) / scale
    upper_bound_offset = (block_size_high - block_size_average) / scale
    ...
    for n in range(number_of_matrices):
        ...
        block_density: float = np.random.uniform(density_low, density_high)
        ...
        while index < dimension - 1:
            ...
            if np.random.uniform(0.0, 1.0) < block_gap_chance:
                ...
                if dimension - (current_block_size + index) < block_size_low:
                ...
                if current_block_size + index >= dimension:
                    ...
                    if current_block_size < block_size_low:
                ...
                for j in range(current_block_size):
                    ...
                    if np.random.random() < block_density:
                        matrix_array[n][a][a] = np.random.uniform(value_low, value_high)
                    for i in range(j):
                        b = i + index
                        if np.random.random() < block_density:
                            value = np.random.uniform(value_low, value_high)
